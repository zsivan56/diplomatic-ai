...
def render_llm_answer(answer, context, placeholder_ctx=None):
    """
    渲染 LLM 回答和检索依据到 Tabs 中。
    - placeholder_ctx: 如果提供，则在对应 placeholder 中渲染依据；否则就地渲染
    """
    if not answer and not context:
        return

    # 智能解答由调用方在 tab1 中用 st.markdown 渲染
    # 检索依据由调用方在 tab2 中渲染
    # 此函数保留仅为兼容，实际渲染逻辑移至 main 的 tabs 中
